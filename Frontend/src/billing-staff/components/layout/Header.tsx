...
export function Header({ onOpenMobileNav }: { onOpenMobileNav: () => void }) {
  ...
  const results = useMemo(() => {
    ...
    // 2. Invoices matching invoice number, patient name, status, payer, line items
    const inv = invoices.filter((i) => {
      ...
      // FIX: use lineItems and serviceName instead of items and description
      if (i.lineItems.some((item) => item.serviceName.toLowerCase().includes(q))) return true;
      return false;
    });
          ...
          <div>
            ...
            {results.pt.map((p) => (
              <button
                key={p.id}
                onClick={() => {
                  router.push(`/billing-staff/patients/${p.id}`);
                  setQuery("");
            ...
            {results.inv.map((i) => {
              ...
              return (
                <button
                  key={i.id}
                  onClick={() => {
                    router.push(`/billing-staff/billing/invoices/${i.id}`);
                    setQuery("");
                    if (isMobile) setMobileSearchOpen(false);
                  }}
                  className="flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-xs hover:bg-brand-50 transition-colors"
          ...
          <div>
            ...
            {results.rc.map((r) => (
              <button
                key={r.id}
                onClick={() => {
                  router.push(`/billing-staff/receipts/${r.id}`);
                  setQuery("");
                  if (isMobile) setMobileSearchOpen(false);
                }}
            ...
            {results.cl.map((c) => {
              ...
              return (
                <button
                  key={c.id}
                  onClick={() => {
                    router.push(`/billing-staff/insurance`);
                    setQuery("");
                    if (isMobile) setMobileSearchOpen(false);
                  }}
          ...
          <div>
            ...
            {results.sc.map((s) => (
              <button
                key={s.id}
                onClick={() => {
                  router.push(`/billing-staff/settings`);
                  setQuery("");
                  if (isMobile) setMobileSearchOpen(false);
                }}
