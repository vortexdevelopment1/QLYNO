          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[90%] sm:max-w-[85%] whitespace-pre-wrap break-words rounded-2xl px-3 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm ${m.role === "user" ? "bg-brand-600 text-white" : "bg-ink-50 text-ink-800 border border-ink-100"}`}>
                <p>{m.text}</p>
                {m.relatedInvoiceId && (
                  <Link href={`/billing-staff/billing/invoices/${m.relatedInvoiceId}`} className="mt-2 inline-block rounded-md bg-brand-50 px-2.5 py-1 text-xs font-semibold text-brand-700 hover:underline border border-brand-200">View Invoice →</Link>
                )}
                {m.relatedReceiptId && (
                  <Link href={`/billing-staff/receipts/${m.relatedReceiptId}`} className="mt-2 inline-block rounded-md bg-brand-50 px-2.5 py-1 text-xs font-semibold text-brand-700 hover:underline border border-brand-200">View Receipt →</Link>
                )}
              </div>
            </div>
          ))}
