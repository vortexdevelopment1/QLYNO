...
import { StatCard } from "@/billing-staff/components/ui/StatCard";
...
import { filterInvoicesByAccess, filterPendingByAccess } from "@/billing-staff/lib/selectors";
import { NewInvoiceModal } from "@/billing-staff/components/billing/NewInvoiceModal";
import { CollectPaymentModal } from "@/billing-staff/components/billing/CollectPaymentModal";
import { RequestRefundModal } from "@/billing-staff/components/billing/RequestRefundModal";
import { DischargeSettlementModal } from "@/billing-staff/components/billing/DischargeSettlementModal";
import { QuickDailyCollectionModal } from "@/billing-staff/components/billing/QuickDailyCollectionModal";
import { AgeingBarChart } from "@/billing-staff/components/dashboard/AgeingBarChart";
import { PaymentMethodBarChart } from "@/billing-staff/components/dashboard/PaymentMethodBarChart";
import { PendingSourceBarChart } from "@/billing-staff/components/dashboard/PendingSourceBarChart";
import { SCOPE_LABELS } from "@/billing-staff/types";
        ...
        <div className="space-y-6 xl:col-span-2">
            ...
            {outstandingInvoices.length > 0 && (
              <div className="mt-5 border-t border-ink-100 pt-4">
                <div className="mb-2.5 flex items-center justify-between">
                  ...
                  <Link href="/billing-staff/outstanding" className="text-xs font-semibold text-brand-600 hover:underline">
          ...
          <section aria-labelledby="payments-heading" className="rounded-2xl border border-ink-100 bg-white p-4 sm:p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between border-b border-ink-100 pb-3">
              ...
              <Link href="/billing-staff/payments" className="text-xs font-semibold text-brand-600 hover:underline">
            ...
            {failedPayments.length > 0 && (
              <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-xl border border-rose-200 bg-rose-50/80 p-3 text-xs text-rose-900">
                ...
                <Link href="/billing-staff/payments" className="font-bold underline text-rose-700 hover:text-rose-900 shrink-0">
                  Resolve Now
                </Link>
              </div>
            )}
          ...
          <section aria-labelledby="pending-heading" className="rounded-2xl border border-ink-100 bg-white p-4 sm:p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between border-b border-ink-100 pb-3">
              ...
              <Link href="/billing-staff/billing/pending" className="text-xs font-semibold text-brand-600 hover:underline">
        ...
        <div className="space-y-6">
          ...
          <section aria-labelledby="refunds-heading" className="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center justify-between border-b border-ink-100 pb-2">
              ...
              <Link href="/billing-staff/refunds" className="text-xs font-semibold text-brand-600 hover:underline">
                Manage
              </Link>
            </div>
          ...
          {showInsuranceSection && (
            <section aria-labelledby="insurance-heading" className="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
              <div className="mb-3 flex items-center justify-between border-b border-ink-100 pb-2">
                ...
                <Link href="/billing-staff/insurance" className="text-xs font-semibold text-brand-600 hover:underline">
                  View Claims
                </Link>
              </div>
          ...
          <section aria-labelledby="recent-patients-heading" className="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
            ...
            <ul className="space-y-1.5">
              {recentPatients.map(({ invoice, patient }) => (
                <li key={invoice.id}>
                  <Link
                    href={`/billing-staff/patients/${patient?.id}`}
