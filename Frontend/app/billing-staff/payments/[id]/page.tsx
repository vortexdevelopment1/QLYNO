...
import { ReversePaymentModal } from "@/billing-staff/components/billing/ReversePaymentModal";
      ...
      <div className="space-y-4 rounded-xl border border-ink-100 bg-white p-5 shadow-card">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div><p className="text-xs text-ink-400">Invoice</p>
            <button onClick={() => invoice && router.push(`/billing-staff/billing/invoices/${invoice.id}`)} className="font-medium text-brand-600 hover:underline">{invoice?.invoiceNumber}</button>
          </div>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          {receipt && (
            <button onClick={() => router.push(`/billing-staff/receipts/${receipt.id}`)} className="rounded-lg border border-ink-200 px-4 py-2 text-sm font-medium text-ink-700 hover:bg-ink-50">
